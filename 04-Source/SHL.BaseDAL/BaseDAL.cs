...
namespace SHL.Data.Base
{
    public abstract class BaseDAL<T>
    {
        ...
        protected List<T> ReturnList(String Group, string procedure, SqlTransaction oSqlTransaction, ArrayList parameters)
        {
            List<T> returnlist = new List<T>();

            SQLHelper.Group = Group;

            using (SqlDataReader dataReader = SQLHelper.ExecuteReader(oSqlTransaction.Connection, oSqlTransaction, procedure, parameters))
            {
                if (dataReader.HasRows)
                {
                    returnlist = new List<T>();
        ...
        protected T ReturnUnique(String Group, String commandText, SqlTransaction oSqlTransaction, ArrayList parameters)
        {
            SQLHelper.Group = Group;

            T obj = default(T);

            using (SqlDataReader dataReader = SQLHelper.ExecuteReader(oSqlTransaction.Connection, oSqlTransaction, commandText, parameters))
            {
                if (dataReader.HasRows)
                {
                    dataReader.Read();
